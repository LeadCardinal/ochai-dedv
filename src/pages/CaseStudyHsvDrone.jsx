...
const CaseStudyHsvDrone = () => {
  ...
  const bonsaiData = [
    { label: 'Jan', value: 32.0 }, { label: 'Mar', value: 29.8 }, 
    { label: 'May', value: 30.5 }, { label: 'Jul', value: 36.4 }, 
    { label: 'Sep', value: 37.1 }, { label: 'Nov', value: 21.3 }, 
  ...
  const metrics = [
    { label: 'Best Position', value: '9.0', icon: Trophy, color: 'text-yellow-400' },
    { label: 'Avg Position', value: '9.0', icon: Search, color: 'text-blue-400' },
    { label: 'Total Impressions', value: '1.5k+', icon: Eye, color: 'text-indigo-400' },
    { label: 'Time to Rank', value: '<30 Days', icon: Zap, color: 'text-emerald-400' },
  ];
  ...
  return (
    <div className="pt-24 pb-12 min-h-screen bg-slate-950 text-white">
      <Helmet>
        ...
        <meta name="description" content="SEO case study for hsvdrone.com showcasing rapid growth to Google Page 1 rankings (Position 9.0) in less than 30 days." />
      </Helmet>
      ...
      <div className="container mx-auto px-4">
        ...
        <header className="mb-12">
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-8">
            ...
            <div className="flex items-center gap-2 bg-slate-900/50 px-4 py-2 rounded-full border border-slate-800">
              ...
              <span className="text-sm font-medium text-yellow-400">High Velocity SEO</span>
            </div>
          </div>
          ...
          >
            ...
            <p className="text-emerald-400 font-medium">Top 10 Google Search Results Secured in Record Time</p>
          </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {metrics.map((metric, idx) => {
              ...
              return (
                ...
                >
                  <div className={`absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity ${metric.color}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-12">
          ...
          <div className="space-y-8">
            <ChartCard title="Page Ranking Breakdown" subtitle="100% Page 1 Dominance for Target Keywords" color="emerald">
              <div className="h-full flex items-center justify-center">
                 <div className="w-full space-y-4 px-4">
                    <div className="flex items-center gap-4">
                      <span className="w-24 text-sm text-slate-400">Page 1 (Top 10)</span>
                      <div className="flex-1 h-8 bg-slate-800 rounded-full overflow-hidden">
                        ...
                        >
                          <span className="absolute right-2 top-1/2 -translate-y-1/2 text-xs font-bold text-slate-900">100%</span>
                        </motion.div>
                      </div>
                    </div>
        ...
        >
          <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400">Organic Growth (Bonsai) vs. Rapid Deployment (HSV Drone)</p>
            </div>
          ...
          <div className="grid md:grid-cols-2 gap-8 h-[300px]">
             ...
             <div className="relative border border-slate-800 bg-slate-950/50 rounded-xl p-4">
                <div className="absolute top-4 left-4 text-xs font-bold text-indigo-400">reallivebonsai.us</div>
                <ResponsiveSimpleLine data={bonsaiData} color="#6366f1" maxY={50} reversed />
                <div className="absolute bottom-4 right-4 text-xs text-slate-500">Avg Pos: 30.2</div>
             </div>
             
             {/* Right: HSV */}
             <div className="relative border border-slate-800 bg-slate-950/50 rounded-xl p-4">
                <div className="absolute top-4 left-4 text-xs font-bold text-blue-400">hsvdrone.com</div>
                <ResponsiveSimpleLine data={hsvComparisonData} color="#3b82f6" maxY={50} reversed />
                <div className="absolute bottom-4 right-4 text-xs text-slate-500">Avg Pos: 9.0</div>
             </div>
          </div>
        </motion.div>
    ...
    <svg className="w-full h-full overflow-hidden" preserveAspectRatio="none" viewBox="0 0 100 100">
      ...
      {targetLine && (
        <g>
            <line 
            x1="0" 
            y1={reversed ? (targetLine / maxY) * 100 : 100 - (targetLine / maxY) * 100} 
            x2="100" 
            y2={reversed ? (targetLine / maxY) * 100 : 100 - (targetLine / maxY) * 100} 
            stroke="#10b981" 
            strokeWidth="1" 
            strokeDasharray="4" 
            />
            <text x="2" y={(reversed ? (targetLine / maxY) * 100 : 100 - (targetLine / maxY) * 100) - 2} fontSize="4" fill="#10b981">Page 1 Threshold</text>
        </g>
      )}
...
const ResponsiveSimpleLine = ({ data, color, maxY, reversed }) => (
    <svg className="w-full h-full overflow-hidden" preserveAspectRatio="none" viewBox="0 0 100 100">
      ...
      />
      {data.map((p, i) => (
          <circle key={i} cx={(i / (data.length - 1)) * 100} cy={(p.value / maxY) * 100} r="2" fill={color} />
      ))}
    </svg>
);
